...
Tests that simulate clicking the actual confirmation and cancellation dialogs require each IDE's GUI test environment. The current automated tests verify a real `git init` in a temporary folder and the pure state branches. The UI text and follow-up commands can be reviewed in the implementation files mapped above.
